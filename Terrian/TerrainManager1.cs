...
public class TerrainManager1 : MonoBehaviour {
	...
	void Start () {
			...
			tempPos = new Vector2((xPosFloor-1)*initiatWidth,(yPosFloor+1)*initiaLength);
			StartCoroutine(getTexture((int)xPosFloor,(int)yPosFloor));
		print(xPosFloor+"-----------"+yPosFloor);
			//if(heightMap != null)
			//getMainTexture((int)xPosFloor+1,(int)yPosFloor+1);
	...
	public void SetTerrain(Vector2 pos){
		...
		//setTerrain(3000,3000,253,253,0,600);
	}
		...
		for(int i = 0;i < segment.x+1;i++){
			for(int j = 0;j < segment.y+1;j++){
				...
				if(heightMap != null){
					...
					if(i == segment.x)
						tempHeight = GetHeight(heightMap,uvs[index - (int)segment.x-1]);
				/*	if(index < 254)
						tempHeight = GetHeight(heightMap,uvs[sum - 254 + index]);
					else
						tempHeight = GetHeight(heightMap,uvs[index-254]);*/
	...
	private Color GetColor(Texture2D texture,Vector2 uv){
		//int a = Mathf.FloorToInt(texture.width*uv.x);int b = Mathf.FloorToInt(texture.height*uv.y);
		Color color = texture.GetPixel(Mathf.FloorToInt(texture.width*uv.x)-1,Mathf.FloorToInt(texture.height*uv.y));
