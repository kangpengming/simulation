...
public class TerrainManager2 : MonoBehaviour {
	...
	void Start () {
			//textureMap = (Texture2D)Resources.Load("Textures/49$6_105$4");
			Vector3 posPlane = Load.centerPos;
			float x = posPlane.x;
			float z = posPlane.z;
			float xPosFloor = Mathf.Floor(x/initiatWidth);
			float yPosFloor = Mathf.Floor(z/initiaLength);
			float m = xPosFloor*initiatWidth;
			tempPos = new Vector2((xPosFloor-1)*initiatWidth,yPosFloor*initiaLength);
			StartCoroutine(getTexture((int)xPosFloor+1,(int)yPosFloor));
		print(xPosFloor+1+"-----------"+yPosFloor);
			//position = new Vector3(12000,0,0);
			//SetTerrain(new Vector2(12000,0));
	}
...
//		textureMap = (Texture2D)Resources.Load("Textures/49$6_105$4");
		//SetTerrain(300,160,250,250,0,600);
	}
